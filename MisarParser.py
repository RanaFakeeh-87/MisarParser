...
def create_psm_instance():
    if not txt_proj_name.get().strip():
        ...
        psm_instance_file = psm_ecore_file.replace(os.path.basename(psm_ecore_file), 'instance.xmi')
        ...
        if xmlns_xsi and xsi_schemaLocation:
            file_lines = [line.rstrip() for line in open(psm_instance_file)]
            file_lines[1] = file_lines[1].rstrip('>') + ' xmlns:xsi="' + xmlns_xsi + '" xsi:schemaLocation="' + xsi_schemaLocation + '" >'
...
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Selected Studies/1-piggymetrics-master/ProjectArtifacts.csv'
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Selected Studies/JOURNAL/19 - BookStoreApp-Distributed-Application-master/ProjectArtifacts.csv'
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Selected Studies/3-spmia-chapter10-master/ProjectArtifacts.csv'
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Selected Studies/JOURNAL/12-event-sourcing-microservices-example-master/event-sourcing-microservices-example-master/ProjectArtifacts.csv'
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Selected Studies/JOURNAL/15-microservices-sample-master-updated/ProjectArtifacts.csv'
input_file = 'C:/Users/Fujitsu/Desktop/Documents/MyWork/2018/NUHA/Evaluation/train-ticket-master/ProjectArtifacts.csv'

with open (input_file) as file:
    for line in file:
        if line.startswith('app_name'):
            txt_proj_name.insert(0, line.split(';')[1].strip())
        elif line.startswith('app_root_dir'):
            txt_proj_dir.configure(state = 'normal')
            txt_proj_dir.delete(0, 'end')
            txt_proj_dir.insert(0, line.split(';')[1].strip())
            txt_proj_dir.configure(state = 'readonly', readonlybackground = 'white')
        elif line.startswith('psm_ecore_file'):
            txt_psm_ecore.configure(state = 'normal')
            txt_psm_ecore.delete(0, 'end')
            txt_psm_ecore.insert(0, line.split(';')[1].strip())
            txt_psm_ecore.configure(state = 'readonly', readonlybackground = 'white')
        elif line.startswith('docker_compose_files'):
            docker_compose_file = line.split(';')[1].strip() 
            if docker_compose_file not in lst_docker_compose.get(0, 'end'):
                lst_docker_compose.insert('end', docker_compose_file)
        elif line.startswith('app_build_files'):
            app_build_file = line.split(';')[1].strip() 
            if app_build_file not in lst_app_build.get(0, 'end'):
                lst_app_build.insert('end', app_build_file)
        elif line.startswith('ms_build_dirs'):
            module_build_dir = line.split(';')[1].strip() 
            if module_build_dir not in lst_module_build_dir.get(0, 'end'):
                lst_module_build_dir.insert('end', module_build_dir)
        elif line.startswith('ms_build_files'):
            module_build_file = line.split(';')[1].strip() 
            if module_build_file not in lst_module_build.get(0, 'end'):
                lst_module_build.insert('end', module_build_file)
        elif line.startswith('app_config_dirs'):
            app_config_dir = line.split(';')[1].strip() 
            if app_config_dir not in lst_app_config_dir.get(0, 'end'):
                lst_app_config_dir.insert('end', app_config_dir)
